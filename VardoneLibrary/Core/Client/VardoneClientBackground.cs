...
namespace VardoneLibrary.Core.Client
{
    internal class VardoneClientBackground
    {
        ...
        {
            _checkingMessageThread = new Thread(CheckingMessageThread);
        ...
        private void CheckUpdatesOutgoingRequestThread()
        {
            ...
            try
            {
                while (_isCheckUpdatesOutgoingRequestThreadWork)
                {
                    ...
                    var list1 = list;
                    if (outgoingFriends.Any(user => !list1.Contains(user)))
        ...
        private void CheckUpdatesIncomingRequestThread()
        {
            ...
            try
            {
                while (_isCheckUpdatesIncomingRequestThreadWork)
                {
                    ...
                    var list1 = list;
                    if (incomingFriends.Any(user => !list1.Contains(user)))
                        onUpdateIncomingFriendRequestList?.Invoke(false);
            ...
            {
                if (e is UnauthorizedException)
                    _isCheckUpdatesIncomingRequestThreadWork = false;
                else
                    throw;
            }
        }
        ...
        private void CheckingMessageThread()
        {
            ...
            {
                if (e is UnauthorizedException)
                    _isCheckMessageThreadWork = false;
                else
                    throw;
            }
        }
